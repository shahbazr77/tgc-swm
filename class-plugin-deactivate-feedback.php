...
if( ! class_exists( 'SWM_Usage_Feedback') ) {
	
	class SWM_Usage_Feedback {
		
		private $swm_version = '0.9.0';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function __construct( 
			...
			$this->home_url = 'yododesing@gmail.com';
		...
		public function init() {
			global $pagenow;
				...
				add_action( 'wp_ajax_swm_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('Shopify to WooCommerce Migration Plugin Deactivation Notice');
				$headers = array('From: Shopify to WooCommerce Migration <mailer@just-a-fake-from-address.com>');
		...
		public function deactivate_this_plugin() {
			...
			$body['url'] = home_url();
			$body['contact1'] = get_option( 'admin_email_'.$current_user_id );
			
			// Add deactivation form data
			if( false !== get_option( 'swm_deactivation_reason_'.$current_user_id . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'swm_deactivation_reason_'.$current_user_id . $this->plugin_name );
				delete_option('swm_deactivation_reason_'.$current_user_id . $this->plugin_name);
			}
			if( false !== get_option( 'swm_deactivation_details_'.$current_user_id . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'swm_deactivation_details_'.$current_user_id . $this->plugin_name );
				delete_option('swm_deactivation_details_'.$current_user_id . $this->plugin_name);
			}

			if( false !== get_option( 'swm_deactivation_email_'.$current_user_id . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'swm_deactivation_email_'.$current_user_id . $this->plugin_name );
				delete_option('swm_deactivation_email_'.$current_user_id . $this->plugin_name);
			}

			if( false !== get_option( 'swm_deactivation_main_reason_'.$current_user_id . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'swm_deactivation_main_reason_'.$current_user_id . $this->plugin_name );
				delete_option('swm_deactivation_main_reason_'.$current_user_id . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-swm-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'swm_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
                $current_user_id = get_current_user_id();
				$html .= '<div class="swm-goodbye-options"><p>';
				foreach( $form['options'] as $option ) {
					$html .= '<input type="radio" name="swm-goodbye-options" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><div id="swm_additional_content" style="display:none;"><label for="swm-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="swm-goodbye-email" id="swm-goodbye-email" value="'.get_option('admin_email_'.$current_user_id).'" /> (Optional)';
				
				$html .= '<br><label for="swm-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="swm-goodbye-reasons" id="swm-goodbye-reasons" rows="2" style="width:100%"></textarea></div>';
				$html .= '</div><!-- .swm-goodbye-options -->';
			}
			...
			?>
			<div class="swm-goodbye-form-bg"></div>


			<script>
				jQuery(document).ready(function($){
					 $(document).on('change', '#wpb-goodbye-form-swm-shopify-to-wc-migration input[type=radio]', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#swm_additional_content').hide();
						}else{
							$('#swm_additional_content').show();
						}
						
					 });

					$('#wpb-goodbye-link-swm-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-swm-<?php echo esc_attr( $this->plugin_name ); ?>');
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							$.each($('input[name=\'swm-goodbye-options[]\']:checked'), function(){
								values.push($(this).val());
							});
							var email = $('#swm-goodbye-email').val();
							var details = $('#swm-goodbye-reasons').val();
							var deactivate_main_reason = $('input[name="swm-goodbye-options"]:checked').val();
							var data = {
								'action': 'swm_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'swm_goodbye_form' ); ?>',
								'dataType': 'json'
							}
						...
						$('.swm-goodbye-form-bg').on('click',function(){
		...
		public function goodbye_form_callback() {
			//check_ajax_referer( 'swm_goodbye_form', 'security' );
            $current_user_id = get_current_user_id();
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'swm_deactivation_reason_'.$current_user_id . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'swm_deactivation_details_'.$current_user_id . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'swm_deactivation_email_'.$current_user_id . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'swm_deactivation_main_reason_'.$current_user_id . $this->plugin_name, $main_reason );
			}
